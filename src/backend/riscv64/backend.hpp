...
/// RISC-V 64-bit LP64 backend: emits GNU assembler syntax.
class RISCV64Backend : public Backend {
public:
    explicit RISCV64Backend(std::ostream& os = std::cout);

    void set_output(std::ostream& os) override;
    std::ostream& output() override;
    std::string target_name() const override { return "riscv64"; }
...
private:
    ...
    /// Emit a register-immediate operation with a fixed opcode
    void emit_ri(const std::string& op, const std::string& rd,
